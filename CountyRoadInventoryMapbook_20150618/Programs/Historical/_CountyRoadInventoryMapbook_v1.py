...
output = "C:\\TxDOT\\County Road Inventory Mapbooks\\test_20140129"
...
if cycle == "North":
	CountyLayer = "T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\South and North County Shapefiles\\NorthCounties.shp"
elif cycle == "South":
	CountyLayer = "T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\South and North County Shapefiles\\SouthCounties.shp"
else:
	arcpy.AddError("You must use ether 'North' or 'South' for your cycle option")

GridLayer = "Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Map_Index_Grids\\TPP_GIS.MCHAMB1.County_Grids_22K"
...
def createCovers():
	...
	map = arcpy.mapping.MapDocument("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\MXD\\CRI_Covers.mxd")
...
def createLegend():
	...
	map = arcpy.mapping.MapDocument("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\MXD\\Legend.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
	for textElement in arcpy.mapping.ListLayoutElements(map, "TEXT_ELEMENT"):
...
def createGridIndex():
	...
	map = arcpy.mapping.MapDocument("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\MXD\\GridIndexUpdate.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
...
def formatData():
	...
	#pubRDS = "Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Roadways\\TPP_GIS.MCHAMB1.TXDOT_Roadways"
	...
	txdotRoadways = "Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Roadways\\TPP_GIS.MCHAMB1.TXDOT_Roadways"
...
def createMapPages():
	...
	map = arcpy.mapping.MapDocument("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\MXD\\CountyRoadInventoryMaps.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
	for lyr in arcpy.mapping.ListLayers(map):
		...
		# if lyr.name == "TPP_GIS.MCHAMB1.County_Grids_22K":
			# lyr.definitionQuery = "CYCLE = '" + cycle + "' "
			# GridLayer = lyr
	for textElement in arcpy.mapping.ListLayoutElements(map, "TEXT_ELEMENT"):
		if textElement.name == "Year":
			textElement.text = dataYear
...
def createRoadLogReport():
	...
	for Y in countyNames:
		...
		for row in cursor:
			...
			splitter = str(row.getValue("SUM_LEN_OF_SECTION")).split(".")
			decSplit = str(splitter[1])[:3]
			LN = str(splitter[0]) + "." + str(decSplit)
...
def compilePackets():
	...
	shutil.copyfile("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\Documents\\COUNTY_ROAD_CRITERIA.pdf", descriptiveDirectory + os.sep + "COUNTY_ROAD_CRITERIA.pdf")
	shutil.copyfile("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\Documents\\INSTRUCTIONS.pdf", descriptiveDirectory + os.sep + "INSTRUCTIONS.pdf")
	shutil.copyfile("T:\\DATAMGT\\MAPPING\\Mapping Products\\County Road Inventory Mapbooks\\Resources\\Documents\\README_1ST.pdf", descriptiveDirectory + os.sep + "README_1ST.pdf")
