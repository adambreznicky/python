...
#imports
import arcpy, os, string, zipfile, shutil, PyPDF2
...
output = "C:\\TxDOT\\County Road Inventory Mapbooks\\QC_" + suffixDate

#DEPENDENT VARIABLES
if not os.path.exists(output + os.sep + dataYear):
	os.makedirs(output + os.sep + dataYear)
else:
	arcpy.AddError("Output Directory already contains CR Mapbook folder for this year. Please rename or delete before proceeding.")
...
if cycle == "North":
	CountyLayer = "C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\South and North County Shapefiles\\NorthCounties.shp"
elif cycle == "South":
	CountyLayer = "C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\South and North County Shapefiles\\SouthCounties.shp"
else:
...
def copyDataLocal():
	...
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Map_Index_Grids\\TPP_GIS.MCHAMB1.County_Grids_22K", dataGDB + os.sep + "County_Grids_22K")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.SUBFILES", dataGDB + os.sep + "SUBFILES")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Roadways\\TPP_GIS.MCHAMB1.TXDOT_Roadways", dataGDB + os.sep + "TXDOT_Roadways")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.County\\TPP_GIS.MCHAMB1.County", dataGDB + os.sep + "Counties")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.City\\TPP_GIS.MCHAMB1.City", dataGDB + os.sep + "Cities")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Roadways\\TPP_GIS.MCHAMB1.City_Streets", dataGDB + os.sep + "Streets")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.City\\TPP_GIS.MCHAMB1.City_Points", dataGDB + os.sep + "City_Points")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Water\\TPP_GIS.MCHAMB1.Dams", dataGDB + os.sep + "Dam")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Water\\TPP_GIS.MCHAMB1.Water_Bodies", dataGDB + os.sep + "Water_Bodies")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Water\\TPP_GIS.MCHAMB1.Streams", dataGDB + os.sep + "Streams")
	arcpy.Copy_management("Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Park\\TPP_GIS.MCHAMB1.Public_Lands", dataGDB + os.sep + "Public_Lands")
...
def createCovers():
	...
	map = arcpy.mapping.MapDocument("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\MXD\\CRI_Covers.mxd")
	...
	for row in cursor:
		...
		arcpy.AddMessage(row.CNTY_NM + " Cover Complete.")
		arcpy.mapping.ExportToPDF(map,CoversFolder + os.sep + row.CNTY_NM)
	del cursor
	del map
	del dataFrame
...
def createLegend():
	...
	map = arcpy.mapping.MapDocument("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\MXD\\Legend.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
	for textElement in arcpy.mapping.ListLayoutElements(map, "TEXT_ELEMENT"):
...
def createGridIndex():
	...
	map = arcpy.mapping.MapDocument("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\MXD\\GridIndexUpdate.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
...
def formatData():
	...
	#grab data from comanche to route the roadway events
	
	#outboundSHP = "Database Connections\\"+dbaseNAME+"\\TPP_GIS.MCHAMB1.Outbound\\"
	# lastYear = int(dataYear) - 1
	# if arcpy.Exists(outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q4"):
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q4"
	# elif arcpy.Exists(outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q3"):
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q3"
	# elif arcpy.Exists(outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q2"):
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q2"
	# elif arcpy.Exists(outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q1"):
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + dataYear + "_Q1"
	# elif arcpy.Exists(outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + str(lastYear) + "_Q4"):
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + str(lastYear) + "_Q4"
	# else:
		# pubRDS = outboundSHP + "TPP_GIS.MCHAMB1.TXDOT_RTE_RDBD_LN_" + str(lastYear) + "_Q3"
	#this a variable added for QC purposes to utilize the current txdot roadways instead of the recent quarterly published version. it is a temporary placeholder
	pubRDS = dataGDB + os.sep + "TXDOT_Roadways"
	...
	arcpy.MakeRouteEventLayer_lr(pubRDS, "RTE_ID", subfiles, "RTE_ID LINE BMP EMP", tempRTEevents, "", "ERROR_FIELD")
	eventlayer = mapping.Layer(tempRTEevents)
	eventlayer.definitionQuery = """ SUBFILE = '2' AND HIGHWAY_STATUS = '4' AND ADMIN_SYSTEM = '3' AND CONTROLSEC NOT LIKE 'TOL%' """
	...
	#the next 4 groups of code have been added in a recent version of this script after successful runs revealed a need to dissolve rows in the shapefile for each county. this section seems clunky to me, maybe another workflow is in order? 
	...
	#use the unique field to dissolve the roads. this removes multiple features within the final county shapefiles that have all the same attributes. This problem exists because of subfiles records with the same attributes amongst the fields used here but different attributes in fields not used (example being different FC, MPA or UAN)
	#inventory2 = workspace + os.sep + "Roadway_Events_Dissolved"
	...
	#add new fields to ensure the delivered shapefiles have clean/proper field names since esri's tools screwed them up and won't let you just change a field name
	...
	for row in cursor:
		...
		length = row.getValue("SUM_LEN_OF_SECTION")
	...
	#continue with the formatting data process
	#remove unwanted fields. list the ones we want to keep and then delete all fields not on the list
	keepFields = ["OBJECTID", "Shape", "ROUTE", "LENGTH", "SURFACE", "DESIGN", "LANES", "COMMENTS", "COUNTY", "ST_NAME"]
	deleteFields = []
	allFields = arcpy.ListFields(inventory2)
	for aField in allFields:
		if str(aField.name) not in keepFields:
			deleteFields.append(aField.name)
...
def createShapefiles():
	...
	#compile lists of the county names and numbers. 
	names = []
	numbers = []
	cursor = arcpy.SearchCursor(CountyLayer)
	for row in cursor:
		names.append(str(row.CNTY_NM).upper())
		numbers.append(str(row.CNTY_NBR).replace('.0',"")) # adjusted to fix float problem
	del cursor
	...
	for i in countRange:
		...
		shapeFileDefQ = "\"COUNTY\" = "+ str(numbers[i])
...
def createMapPages():
	...
	map = arcpy.mapping.MapDocument("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\MXD\\CountyRoadInventoryMaps.mxd")
	dataFrame = arcpy.mapping.ListDataFrames(map)[0]
	for lyr in arcpy.mapping.ListLayers(map):
...
def combineMapbooks():
	...
	#iterate through the counties within the dictionary and compile all the page numbers up until its maximum number of pages from the dictionary value
...
def formatRoadLog():
	...
	#clean up the intersect of grids which overlap neighbor counties. recalculate the new segment lengths since the intersect broken the linework at the county and grid boundaries.
	...
	thesePages = []
	dictionary = {}
	#use the searchCursor to compile all the page numbers for each route ID into a list, and then use that list as the value with the route ID as the key in the dictionary
	for row in cursor:
...
def createRoadLogReport():
	...
	#compile a list of the county names
	countyNames = []
	cursor = arcpy.SearchCursor(CountyLayer)
	for row in cursor:
		name = row.getValue("CNTY_NM")
		if str(name) not in countyNames:
			countyNames.append(str(name))
	del cursor
	arcpy.AddMessage("County name list compiled.")
	
	#iterate through the list of county names to create a report for each county
	for Y in countyNames:
		#import the dimensions for the report and create variable to determine the maximum measurements for that page size
		from reportlab.lib.pagesizes import letter
		width, height = letter
		#create a variable for the 'flowable' data drawing area on the report. this variable draws the location where the road summary data is inserted into the report template 
		...
		def thecanvas(c, doc):
			from reportlab.lib.pagesizes import letter
			width, height = letter
			#the template/canvas is object oriented. this is a list of all the objects, where they are to be drawn, and all the defining information which draws them 
			...
			c.drawString(3*inch,.4*inch, "3 = Blvd.")
		...
		#search the formatted road log feature class via a cursor. query the feature class for the county being reported, sort, create a list of the attributes from each row, and apply the list to a list of all the data rows
...
def compilePackets():
	...
	shutil.copyfile("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\Documents\\COUNTY_ROAD_CRITERIA.pdf", descriptiveDirectory + os.sep + "COUNTY_ROAD_CRITERIA.pdf")
	shutil.copyfile("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\Documents\\\\INSTRUCTIONS.pdf", descriptiveDirectory + os.sep + "INSTRUCTIONS.pdf")
	shutil.copyfile("C:\\TxDOT\\County Road Inventory Mapbooks\\Resources\\Documents\\\\Documents\\README_1ST.pdf", descriptiveDirectory + os.sep + "README_1ST.pdf")
	...
	#create a list of all the county names
	countyNames = []
	cursor = arcpy.SearchCursor(CountyLayer)
	for row in cursor:
