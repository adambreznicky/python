...
output = "C:\\TxDOT\\County Road Inventory Mapbooks\\test\\QC_North_" + suffixDate
...
def formatData():
    ...
    for row in cursor:
        ...
        newlen = feat.length * .000621371
        newerlen = format(float(newlen), '.3f')

        row.setValue("ROUTE", CS)
        row.setValue("LENGTH", newerlen)
...
def formatRoadLog():
    ...
    for row in cursor:
        ...
        newlen = feat.length * .000621371
        newerlen = format(float(newlen), '.3f')
        row.setValue("LEN_OF_SECTION", newerlen)
        cursor.updateRow(row)
        ...
        if int(frst) != int(scnd) and thrd == frth:
            cursor.deleteRow(row)
        elif int(frst) == int(scnd) and thrd != frth:
            cursor.deleteRow(row)
        elif int(frst) != int(scnd) and thrd != frth:
            goodname = row.CNTY_NM_12
            row.setValue("CNTY_NM", goodname)
            cursor.updateRow(row)
        else:
            pass
