...
def dictionary():
    ...
    cursor = arcpy.da.SearchCursor(roadways, ["RTE_ID", "FULL_ST_NM"], query)
    for row in cursor:
        roads[row[0]] = row[1]
    del cursor
    del row
    ...
    cursor = arcpy.da.SearchCursor(subfiles, ["OBJECTID", "RTE_ID", "CONTROLSEC", "FC", "STREET_NAME", "BEGIN_TERMINI", "END_TERMINI"], query)
    for row in cursor:
