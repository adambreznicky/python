...
def overlap(d):
    try:
        ...
        arcpy.AddMessage("Done overlap errors.")
...
def routeopen(d):
    ...
    openstatus = {}
    counter = 0
    whereto = """ ( RDBD_TYPE = 'CNCTR-GS' AND RTE_CLASS = '1' ) OR( (RTE_CLASS = '2' OR RTE_CLASS = '3') AND RDBD_TYPE <> 'CONNECTOR' AND RDBD_TYPE <> 'OTHER' AND RDBD_TYPE <> 'RAMP' AND RDBD_TYPE <> 'TURNAROUND' ) OR RTE_ID = '183A-AG' OR RTE_ID = '183A-KG' OR RTE_ID = '183A-XG' OR (RTE_NM = '183A' AND RDBD_TYPE = 'CNCTR-GS') """
    ...
    for row in cursor:
        ...
        try:
            ...
            if math.isnan(Mmin):
                ...
                errorinfo.append("")
                errorinfo.append(len)
                errorinfo.append("Minimum measure is NaN. OBJECTID: " + oid)
                errors.append(errorinfo)
                counter += 1
            if math.isnan(Mmax):
                errorinfo = []
                ...
                errorinfo.append("")
                errorinfo.append(len)
                errorinfo.append("Maximum measure is NaN. OBJECTID: " + oid)
                errors.append(errorinfo)
                counter += 1
            Mmin = 0
            Mmax = 0
            print id + " has one of the measure issues!!!"
            errorinfo = []
            errorinfo.append(id)
            errorinfo.append("")
            errorinfo.append("")
            ...
            errorinfo.append("")
            errorinfo.append(len)
            errorinfo.append("Measure problems. OBJECTID: " + oid)
            errors.append(errorinfo)
            counter += 1
...
def onsystem(d):
    ...
    counter = 0
    attexceptions = ['SHOSR-KG', 'SRT-XG', 'SRT-AG']
    atterrors = []
    measerrors = []
        ...
        if row.RTE_ID not in attexceptions:
            ...
            if row.RTE_ID == 'SHOSR-KG':
                ...
                if row.RTE_NBR != '121':
                    ...
                    errorinfo.append(row.RTE_NBR)
                    errorinfo.append("")
                    errorinfo.append("RTE_NBR should be 121")
                    atterrors.append(errorinfo)
                    counter += 1
                if row.RDBD_TYPE != rdbd:
                    errorinfo = []
...
def offsysatt(d):
    ...
    tollid = []
    cursor = arcpy.SearchCursor(tolltable)
    for row in cursor:
        id = row.RTE_ID
        os = row.ON_SYSTEM
        if os != 1 and id not in tollid:
            tollid.append(id)
    del cursor
    del row
...
def assemblereport(check1, check2, check3, check4, check5, check6, check7, check8, check9):
    ...
    started = []
    if check2 == "true":
        p = multiprocessing.Process(name='OverlapCheck', target=overlap, args=(d,))
        p.start()
        # started.append(p)
    if check3 == "true":
        p = multiprocessing.Process(name='RteOpenCheck', target=routeopen, args=(d,))
        p.start()
        started.append(p)
    if check4 == "true":
        p = multiprocessing.Process(name='MeasureLengthCheck', target=measurelength, args=(d,))
        p.start()
        started.append(p)
    if check5 == "true":
        p = multiprocessing.Process(name='SubfileLengthCheck', target=subfilelength, args=(d,))
        p.start()
        started.append(p)
    if check6 == "true":
        p = multiprocessing.Process(name='RemoveVerticesCheck', target=removevertices, args=(d,))
        p.start()
        started.append(p)
    if check7 == "true":
        p = multiprocessing.Process(name='OnSystemCheck', target=onsystem, args=(d,))
        p.start()
        started.append(p)
    if check8 == "true":
        p = multiprocessing.Process(name='OffSystemAttCheck', target=offsysatt, args=(d,))
        p.start()
        started.append(p)
    if check9 == "true":
        p = multiprocessing.Process(name='ScreenLinesCheck', target=screenlines, args=(d,))
        p.start()
        # started.append(p)
    for i in started:
        print "Waiting on: " + i.name
        i.join()
    ...
    if check2 == "true":
        ...
        counter = 0
        errors = []
        comperrors = workspace + "\\Overlap_Working.gdb\\City_OverlapErrors"
        cursor = arcpy.SearchCursor(comperrors)
        for row in cursor:
    ...
    if check9 == "true":
        ...
        Screen_Lines = workspace + "\\SL_Working.gdb\\Screen_Lines"
        Screen_Lines_Summarized_dbf = workspace + "\\SL_Working.gdb\\summarized"
        exceptions = {}
