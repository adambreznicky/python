...
def copylocal():
    ...
    arcpy.Copy_management(txdotcnty, counties)
    arcpy.SpatialJoin_analysis(txdotroads, districts, roadways)


def overlap():
    arcpy.Select_analysis(roadways, database + "\\OffSystem_Roads", """ RTE_CLASS = '2' OR RTE_CLASS = '3' """)
    arcpy.Union_analysis([cities, counties], database + "\\City_Union")
    arcpy.Intersect_analysis([roadways, database + "\\City_Union"], workspace + "\\City_OverlapErrors.shp")

    errors = []
    cursor = arcpy.UpdateCursor(workspace + "\\City_OverlapErrors.shp")
    counter = 0
    for row in cursor:
        if row.RTE_CLASS == 2 and row.TX_CITY_NB is None:
            cursor.deleteRow(row)
        elif row.RTE_CLASS == 3 and row.TX_CITY_NB is not None:
            cursor.deleteRow(row)
        else:
            geom = row.shape
...
def subfilelength():
    ...
    for row in cursor:
        ...
        if id not in dictionary.keys() and id is not None:
            ...
            errorinfo.append("NO ROUTE ID. RTE_ID is NULL")
            errors.append(errorinfo)
    del cursor
    del row
