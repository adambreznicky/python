...
txdotroads = "Database Connections\\Direct_Comanche.sde\\TPP_GIS.APP_TPP_GIS_ADMIN.Roadways\\TPP_GIS.APP_TPP_GIS_ADMIN.TXDOT_Roadways"
txdotsubs = "Database Connections\\Direct_Comanche.sde\\TPP_GIS.APP_TPP_GIS_ADMIN.SUBFILES"
txdotcity = "Database Connections\\Direct_Comanche.sde\\TPP_GIS.APP_TPP_GIS_ADMIN.City\\TPP_GIS.APP_TPP_GIS_ADMIN.City"
txdotdist = "Database Connections\\Direct_Comanche.sde\\TPP_GIS.APP_TPP_GIS_ADMIN.District\\TPP_GIS.APP_TPP_GIS_ADMIN.District"
...
def removevertices():
    ...
    cursor = arcpy.UpdateCursor(txdotroads)
    for row in cursor:
        geom = row.shape
            ...
            for pnt in part:
                if srtpnt == 0:
                    ...
                    if row.RDBD_TYPE == "LG" or row.RDBD_TYPE == "XG":
                        if math.isnan(m) and pnt.ID != len(part):
                            errorinfo = []
                            errorinfo.append(row.OBJECTID)
                            errorinfo.append(row.RTE_ID)
                            errorinfo.append(row.DIST_NM)
                        ...
                        if newMiles >= .002 and pnt.ID != len(part):
                            nupnt = arcpy.Point(x, y, 0, m)
                            ary.add(nupnt)
                            lastX = x
                            lastY = y
                            lastM = m
                        elif newMiles < .002 and pnt.ID == len(part):
                            ary.remove(-1)
                            nupnt = arcpy.Point(x, y, 0, m)
                            ary.add(nupnt)
                        elif newMiles >= .002 and pnt.ID == len(part):
