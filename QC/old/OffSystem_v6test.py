...
txdotroads = "C:\\TxDOT\\QC\\OffSystem\\tester\\copy.gdb\\txdot_roadways"
txdotsubs = "C:\\TxDOT\\QC\\OffSystem\\tester\\copy.gdb\\SUBFILES"
                ...
                if srtpnt == 0:
                    ...
                    if row.RDBD_TYPE == "LG" or row.RDBD_TYPE == "XG":
                        if math.isnan(m) and pnt.ID != len(part):
                            ...
                            errorinfo.append(row.DIST_NM)
                            errorinfo.append(row.DIST_NBR)
                            errorinfo.append("Has vertex with zero measure, Re-apply measures.")
                        ...
                        if m > lastM:
                            ...
                            errorinfo.append(row.DIST_NM)
                            errorinfo.append(row.DIST_NBR)
                            errorinfo.append("Has vertex with non-decreasing measure, Re-apply measures.")
                            if errorinfo not in errors:
                                errors.append(errorinfo)
                                counter += 1
                        ...
                        if m < lastM:
                            ...
                            errorinfo.append(row.DIST_NM)
                            errorinfo.append(row.DIST_NBR)
                            errorinfo.append("Has vertex with non-increasing measure, Re-apply measures.")
                            if errorinfo not in errors:
                                errors.append(errorinfo)
                                counter += 1
                        newM = (math.sqrt((abs(x - lastX)) ** 2 + (abs(y - lastY)) ** 2))
...
def assemblereport():
    ...
    multisheet.write(line, 6, "The following Object IDs are multipart features or vertex measure errors")
    line += 1
