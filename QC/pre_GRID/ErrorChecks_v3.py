...
qcfolder = check0
workspace = qcfolder + "\\v3_EOYclean" + today
...
outsideTexas = ["148AA6011", "009AA2040", "056AA0240", "148AA0594", "148AA6010", "059AA0100"]
...
def onsystem():
    ...
    atterrors = []
    measerrors = []
    query = """ RTE_CLASS ='1' AND RDBD_TYPE <> 'RAMP' AND RDBD_TYPE <> 'TURNAROUND' AND RDBD_TYPE <> 'CONNECTOR' AND RDBD_TYPE <> 'OTHER'"""
    cursor = arcpy.SearchCursor(roadways, query)
    for row in cursor:
    ...
    for row in cursor:
        ...
        if sub == 2:
            ...
            if idcs != cs:
                ...
                errorinfo.append(cs)
                errorinfo.append("RTE_ID prefix and number should match CONTROLSEC.")
                errors.append(errorinfo)
                counter += 1
            ...
            if cs is not None:
                ...
                cslen = 0
            if cslen != 6 and cslen != 7:
                errorinfo = []
                errorinfo.append(id)
                errorinfo.append(dist)
                errorinfo.append(cs)
                errorinfo.append(cslen)
                errorinfo.append("CONTROLSEC should be 6 digits long. (Or 7 containing a '-' if a turnback)")
                errors.append(errorinfo)
                counter += 1
            elif cslen == 7:
                if "-" not in cs:
                    ...
                    errorinfo.append(cs)
                    errorinfo.append(cslen)
                    errorinfo.append("7 digit CONTROLSEC should contain a '-' (dash).")
                    errors.append(errorinfo)
                    counter += 1
            if city > 0:
                pass
...
def screenlines():
    ...
    try:
        ...
        arcpy.FeatureClassToFeatureClass_conversion(txdotroads, slbase, "OnSystem_Roadways", """(RTE_CLASS = '1' OR RTE_CLASS = '6') AND RTE_OPEN = 1""")
        ...
        arcpy.LocateFeaturesAlongRoutes_lr(Screen_Line_Intersect_shp, OnSystem_Roadways, "RTE_ID", "0.002 Meters", Screen_Line_Result_dbf, Output_Event_Table_Properties)
...
def email():
    ...
    TEXT = "You are receiving this email because your district(s) was listed within the error checks run on " \
           ...
           + tdrive + "!!Reminder about screen lines and city limit exceptions!!\n\nCITY LIMITS OVERLAPS:\nIf you visit a city overlap error and the geometry unjustifiably violates our data rules, fix it!\nyou visit an overlap error and the boundary overlap is justified because of a bad city boundary or logical connectivity reasons please email me the ROUTE ID. I will confirm the exception and add it to the exception table so that it no longer appears in the error report.\n\nSCREEN LINES:\nIf you visit a screen line error and the measures are incorrect for the linework, fix it!\nAlternatively, if you visit a screen line error and the measures have permanently changed resulting in a new acceptable difference between the roadbed measures please email me the SCREEN LINE ID and the new DIFFERENCE. I will alter the exceptions table so that it no longer appears in the error report.\n" + "\n\nLove, Adam"
