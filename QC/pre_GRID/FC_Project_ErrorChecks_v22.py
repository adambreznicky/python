...
IDlist = "C:\\TxDOT\\Scratch\\SUBFILES_CheckTermini_12_5.dbf"
IDlist2 = "C:\\TxDOT\Scratch\\assigned.dbf"
...
def dictionary():
    ...
    query = ""
    cursor = arcpy.da.SearchCursor(IDlist, ["RTE_ID"])
    trigger = 0
    for row in cursor:
        id = row[0]
        if trigger == 0:
            ...
            query = query + " OR RTE_ID = '" + str(id) + "'"
    del cursor
    del row
    print "Updated acquired."

    cursor = arcpy.da.SearchCursor(IDlist2, ["RTE_ID"])
    for row in cursor:
        id = row[0]
        query = query + " OR RTE_ID = '" + str(id) + "'"
    del cursor
    ...
    cursor = arcpy.da.SearchCursor(roadways, ["RTE_ID", "FULL_ST_NM"], "RTE_CLASS = '3' AND (" + query + ")")
    for row in cursor:
        roads[row[0]] = row[1]
    del cursor
    del row
    ...
    cursor = arcpy.da.SearchCursor(subfiles, ["OBJECTID", "RTE_ID", "CONTROLSEC", "FC", "STREET_NAME", "BEGIN_TERMINI", "END_TERMINI"], "SUBFILE = 3 AND (" + query + ")")
    for row in cursor:
